...
export async function generateSummaryReport(src:SummarySource,usage:{sessionId?:string;shareCode?:string},onProgress?:(ratio:number)=>void):Promise<SummaryAnalysisGeneration>{
  ...
  const summaryDossier=await withAiUsageContext({sessionId:usage.sessionId,shareCode:usage.shareCode,characterName:src.name,stage:'summary_psychology'},()=>buildSummaryDossier(dossierInput,dossierProgress));
  ...
  return withAiUsageContext({sessionId:usage.sessionId,shareCode:usage.shareCode,characterName:src.name,stage:'summary_teaser'},()=>generateSummary(summaryInput,src,writeProgress));
}
