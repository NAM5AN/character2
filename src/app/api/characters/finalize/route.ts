...
async function generateSummary(input:string,body:z.infer<typeof requestSchema>,review:any):Promise<SummaryAnalysisGeneration>{
  ...
  for(let attempt=0;attempt<3;attempt++){
    ...
    const raw=await askOpenAIJson({instructions:SUMMARY_SYSTEM,schema:summaryAnalysisRawSchema,maxOutputTokens:3000,input:`${input}${retry}`});
