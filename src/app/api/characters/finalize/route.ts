...
분석 규칙:
- 공개 프로필, 비밀 프로필, 오너 검수, 20문항의 답변과 이유를 모두 사용할 수 있습니다.
...
사용자에게 보이는 summary 6개 필드는 각각 130~220자를 목표로 2~4개의 완결된 문장으로 작성하세요. 같은 행동이나 같은 결론을 카드마다 반복하지 마세요.
- outerSelf: 겉으로 다른 사람이 체감하는 인상과, 그 인상이 단순한 성격 라벨로 설명되지 않는 지점을 보여주세요.
...
- misunderstoodPoint: 겉으로는 한 의미로 오해받기 쉽지만 실제 기능은 다르게 읽히는 지점을 하나 골라 설명하세요.
...
function normalize(raw:z.infer<typeof summaryAnalysisRawSchema>,body:z.infer<typeof requestSchema>,review:any){
  ...
  return {
    ...
    summary:{
      outerSelf:text(s.outerSelf),
      innerSelf:text(s.innerSelf),
      conflictStyle:text(s.conflictStyle),
      affectionStyle:text(s.affectionStyle),
      misunderstoodPoint:text(s.misunderstoodPoint),
      hiddenPattern:text(s.hiddenPattern),
    },
...
function shortSummaryFields(summary:SummaryAnalysisGeneration['summary']){
  return teaserKeys.flatMap(key=>{
    ...
    return !value||value.trim().length<110?[`${key} ${value?.trim().length||0}자`]:[];
  });
}
...
async function generateSummary(input:string,body:z.infer<typeof requestSchema>,review:any):Promise<SummaryAnalysisGeneration>{
  ...
  for(let attempt=0;attempt<2;attempt++){
    const retry=attempt===0?'':`\n\n이전 생성은 JSON 형식 또는 공개 요약 품질 점검에 걸렸습니다. 이번에는 사용자에게 보이는 oneLineSummary와 summary 6개 필드를 최우선으로 새로 작성하세요. 각 summary 필드는 130~220자를 목표로 하고 110자보다 짧아지지 않게 충분한 맥락을 담으세요. 원자료 한 문장을 표현만 바꾸지 말고 서로 다른 단서를 연결한 해석을 반드시 포함하세요. misunderstoodPoint와 hiddenPattern도 빠뜨리지 마세요. evidencePack은 빈 객체 {}로 출력해도 됩니다. 이전 출력을 수리하지 말고 원자료에서 새로 작성하세요. 점검 내용: ${last}`;
    try{
      ...
      if(parsed.success){
        ...
        if(shortFields.length&&attempt===0){
          last=`공개 요약 필드가 권장 최소 110자보다 짧음: ${shortFields.join(', ')}`;
          continue;
        }
        return parsed.data;
      }
