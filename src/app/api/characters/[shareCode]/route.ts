...
export async function POST(request:Request,context:{params:Promise<{shareCode:string}>}){
  try{
    ...
    // New lazy-detail characters: return the paid report from cache if it was
    // already generated by a previous paid unlock.
    if(bundle.detail){
    ...
    // Legacy characters were created before lazy generation and already have a
    // complete detailed analysis in the old passport. Do not spend AI again.
    ...
    // Paid code is valid and no cached detail exists: this is the first moment
    // the long Claude report is actually generated.
    const analysis=await generatePaidDetail(bundle.seed);
