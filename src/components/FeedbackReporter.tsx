  ...
  return <>
    ...
    {open&&<div className="modal-backdrop feedback-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget&&!busy)setOpen(false)}}>
      <div className="modal feedback-modal" role="dialog" aria-modal="true" aria-labelledby="feedback-title">
        ...
        <div className="field"><label className="label"><span className="label-text">내용</span><textarea className="input feedback-textarea" disabled={busy} maxLength={5000} value={content} onChange={e=>setContent(e.target.value)} placeholder="어떤 문제가 있었는지, 또는 어떻게 바뀌면 좋을지 적어주세요." /></label></div>
        <div className="field"><label className="label"><span className="label-text">이미지·영상 첨부 <span className="muted">({files.length}/{MAX_FILES})</span></span><input ref={inputRef} type="file" accept="image/*,video/*" multiple hidden onChange={e=>addFiles(e.target.files)} /></label><button type="button" className="btn" disabled={busy||files.length>=MAX_FILES} onClick={()=>inputRef.current?.click()}>파일 선택</button></div>
