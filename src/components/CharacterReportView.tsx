...
export function CharacterReportView({preview,creatorEditToken}:{preview:CharacterReportPreview;creatorEditToken?:string}){
  ...
  return <>
    ...
    {!detail&&<section className="card" style={{marginTop:24,padding:'34px 28px',overflow:'hidden',position:'relative'}}>
      ...
      <div className="tags" style={{marginBottom:18}}>{['유형별 해석 원문','핵심 가치·욕구','두려움','관계·갈등 패턴','오해받는 지점','캐릭터의 모순','상세 통합 리포트'].map(x=><span className="tag" key={x}>{x}</span>)}</div>
      ...
      ].map(([title,t])=><div key={title} style={{border:'1px solid var(--line)',borderRadius:16,padding:'18px 20px',background:'white'}}><strong>{title}</strong><p style={{margin:'10px 0 0',lineHeight:1.7,filter:'blur(6px)',opacity:.58,userSelect:'none'}}>{t} {t}</p></div>)}<div style={{position:'absolute',inset:0,background:'linear-gradient(180deg,transparent 18%,rgba(255,253,248,.28) 55%,rgba(255,253,248,.82) 100%)',pointerEvents:'none'}}/></div>
    ...
    {detail&&<div id="paid-detail-report" style={{scrollMarginTop:90,marginTop:34}}>
      ...
      <div className="result-grid" style={{marginTop:20}}>
        <section className="result-block"><h3>겉으로 보이는 모습 · 원문</h3><p>{detail.analysis.outerSelf}</p></section><section className="result-block"><h3>실제 내면 · 원문</h3><p>{detail.analysis.innerSelf}</p></section><section className="result-block"><h3>갈등 방식 · 원문</h3><p>{detail.analysis.conflictStyle}</p></section><section className="result-block"><h3>애정 표현 · 원문</h3><p>{detail.analysis.affectionStyle}</p></section>
        <section className="result-block"><h3>핵심 가치</h3><div className="tags">{detail.analysis.coreValues.map(x=><span className="tag" key={x}>{x}</span>)}</div></section><section className="result-block"><h3>핵심 욕망</h3><div className="tags">{detail.analysis.desires.map(x=><span className="tag" key={x}>{x}</span>)}</div></section><section className="result-block"><h3>두려워하는 것</h3><div className="tags">{detail.analysis.fears.map(x=><span className="tag" key={x}>{x}</span>)}</div></section>
        <section className="result-block"><h3>쉽게 오해받는 부분</h3><p>{detail.analysis.misunderstoodPoints.map(x=>`• ${x}`).join('\n')}</p></section><section className="result-block"><h3>캐릭터의 모순</h3><p>{detail.analysis.contradictions.map(x=>`• ${x}`).join('\n')}</p></section><section className="result-block"><h3>AI가 발견한 흥미로운 지점</h3><p>{detail.analysis.interestingPoints.map(x=>`• ${x}`).join('\n')}</p></section>
      </div>
      <section className="card" style={{marginTop:22,padding:'32px'}}><div className="eyebrow">Deep report</div><h2 style={{fontSize:'clamp(28px,4vw,42px)',marginTop:10}}>통합 상세 해석</h2>{detail.analysis.detailedReport?<p style={{fontSize:17,lineHeight:1.9,whiteSpace:'pre-line',marginBottom:0}}>{detail.analysis.detailedReport}</p>:<p className="muted">이 캐릭터는 상세 리포트 기능 추가 이전 버전으로 생성되어 기존 유형별 원문만 제공됩니다.</p>}</section>
