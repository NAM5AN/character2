...
export function AnalyzeFlow(){
  ...
  function renderResponseControls(){
    ...
    if(type==='relationship_matrix')return <div style={{marginTop:20}}>{config.rows.map(row=>{const choices=config.rowOptions?.[row]?.length?config.rowOptions[row]:config.columns;return <div key={row} style={{padding:'14px 0',borderBottom:'1px solid var(--line)'}}><div className="label" style={{marginBottom:8}}>{row}</div><div className="options">{choices.map(choice=><button disabled={busy} key={choice} className={`option ${matrixAnswers[row]===choice?'selected':''}`} onClick={()=>setMatrixAnswers(current=>({...current,[row]:choice}))}>{choice}</button>)}</div></div>})}</div>;
