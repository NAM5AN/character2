...
export function AnalyzeFlow(){
  ...
  return <>
    ...
    {stage==='review' && draft && <div className="stack">
      ...
      <div className="card"><h3>AI 추론 검수</h3><p className="muted">캐릭터 자체에 대한 해석만 표시합니다. 여기서 ‘아님’을 누른 내용은 이후 질문과 최종 캐해에서 제외됩니다.</p>{draft.aiInferences.map(x=><div className="inference" key={x.id}><div className="inference-top"><p>{x.text}</p><span className="muted">{Math.round(x.confidence)}%</span></div><div className="pills"><button className={`pill ${x.ownerVerdict==='confirmed'?'active':''}`} onClick={()=>verdict(x.id,'confirmed')}>맞음</button><button className={`pill ${x.ownerVerdict==='ambiguous'?'active':''}`} onClick={()=>verdict(x.id,'ambiguous')}>애매함</button><button className={`pill ${x.ownerVerdict==='rejected'?'active':''}`} onClick={()=>verdict(x.id,'rejected')}>아님</button></div></div>)}</div>
