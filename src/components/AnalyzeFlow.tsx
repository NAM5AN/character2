...
export function AnalyzeFlow(){
  ...
  function buildCurrentAnswer(){if(!question)return null;const answer=(custom.trim()||selected).trim();if(!answer)return null;const reasonText=reason.trim();return {order:question.order,question:question.question,answer,...(reasonText?{reason:reasonText}:{}),branchContext:{category:question.category,mode:question.mode,format:question.format,targetHook:question.targetHook,hypothesis:question.hypothesis}} satisfies InterviewAnswer}
