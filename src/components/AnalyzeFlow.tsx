...
export function AnalyzeFlow(){
  ...
  function renderResponseControls(){
    ...
    if(type==='condition_followup'){
      ...
      return <><div className="field"><label className="label">기본 상황</label><div className="options">{baseChoices.map(o=><button disabled={busy} key={o} className={`option ${selected===o?'selected':''}`} onClick={()=>{setSelected(o);setCustom('')}}>{o}</button>)}<div className={`option ${baseCustom?'selected':''}`} style={{display:'block',cursor:'text'}}><strong style={{display:'block',marginBottom:8}}>직접 입력</strong><input disabled={busy} className="input" style={{width:'100%',padding:'9px 10px'}} placeholder="이 상황에서 할 행동을 직접 적어주세요." value={baseCustom} onChange={e=>{setCustom(e.target.value);setSelected('')}}/></div></div></div><div className="field"><label className="label">{config.prompt2}</label><div className="options">{shiftedChoices.map(o=><button disabled={busy} key={o} className={`option ${secondary===o?'selected':''}`} onClick={()=>setSecondary(o)}>{o}</button>)}<div className={`option ${shiftedCustom?'selected':''}`} style={{display:'block',cursor:'text'}}><strong style={{display:'block',marginBottom:8}}>직접 입력</strong><input disabled={busy} className="input" style={{width:'100%',padding:'9px 10px'}} placeholder="조건이 바뀌었을 때 할 행동을 직접 적어주세요." value={shiftedCustom} onChange={e=>setSecondary(e.target.value)}/></div></div></div></>;
    }
