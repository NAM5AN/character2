...
export function AnalyzeFlow(){
  ...
  function clearProgressState(){setStage('input');setName('');setProfileText('');setSecretProfileText('');setDraft(null);setAnswers([]);setQuestion(null);setHistory([]);setActiveQuestionIndex(0);resetResponseDraft();setBusy(false);setError('');setResult(null);batchRequests.current.clear()}
  ...
  function renderResponseControls(){
    ...
    if(type==='condition_followup')return <><div className="field"><label className="label">기본 상황</label><div className="options">{question.options.map(o=><button disabled={busy} key={o} className={`option ${selected===o?'selected':''}`} onClick={()=>setSelected(o)}>{o}</button>)}</div></div><div className="field"><label className="label">{config.prompt2}</label><div className="options">{question.options.map(o=><button disabled={busy} key={o} className={`option ${secondary===o?'selected':''}`} onClick={()=>setSecondary(o)}>{o}</button>)}</div></div></>;
