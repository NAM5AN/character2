...
- 각 문단 본문에는 하이라이트를 정확히 1곳 넣으세요. 안내문이 던진 주제에 가장 직접적으로 답하는 핵심 결론이어야 하며 생략하지 마세요.
- 강조할 것은 "이 캐릭터만의 작동 원리·조건·모순이 서술어까지 완결된 구절"입니다. 명사구에서 끊지 말고 조사·서술어를 함께 감싸세요.
- 예: 통증보다 **반응이 끊기는 침묵을 더 크게 위협으로 받아들여요**. / **침착함이 결핍을 가리는 기술이라서** 가까이서 볼수록 다른 집중력이 드러나요.
- 여러 문장을 통째로 감싸지 마세요. 한 번에 감싸는 길이는 대략 12~45자를 권장하되, 의미가 완결되는 것이 길이보다 우선입니다.
- 단순 사실, 뻔한 성격 라벨, 이미 안내문에서 말한 내용을 다시 감싸지 마세요.
- 한 문단에 두 곳 이상을 강조하지 마세요.
