...
export const DETAIL_REPORT_VERSION = 'detail-analysis/4.1' as const;
...
// domains/confidence are helper metadata for the second pass. They are deliberately
// tolerant strings: a useful analysis must not fail only because the model wrote
// a Korean label instead of a hard-coded English enum.
const evidenceCardSchema = z.object({
  ...
  domains: z.array(z.string().min(1).max(50)).min(1).max(4),
...
- 가능하면 하나의 thesis에 서로 다른 두 개 이상의 근거를 연결하세요. 근거가 하나뿐이면 confidence에 낮은 확신임을 표시하고 과도한 심리 원인을 만들지 마세요.
...
좋은 카드는 추상 성격표가 아니라, 구체적 행동들이 어떤 하나의 기준으로 연결되는지 설명합니다.`;
...
- 구체성을 피한다는 이유로 누구에게나 붙일 수 있는 추상 문장만 쓰지 마세요.

최종 결과가 해야 하는 일:
- 각 주요 해석에는 가능하면 이 캐릭터만의 구체적인 행동·상황·관계 조건을 한두 개 자연스럽게 녹이세요.
...
- 독특한 디테일은 그 자체를 나열하지 말고 캐릭터의 인식 방식이나 선택 원리를 보여줄 때만 사용하세요.
...
function asList(value:unknown){
  ...
  return raw.map(asInlineText).map(x=>clipText(x,80)).filter(x=>x.length>=8).slice(0,5);
}
...
function contextString(value:unknown,key:string){
  const record=asRecord(value);return typeof record[key]==='string'?String(record[key]):'';
}
...
function semanticAnswer(answer:z.infer<typeof interviewAnswerSchema>):SemanticAnswer{
  ...
  text=text.replace(/\b\d{1,3}\s*\/\s*(?:5|100)\b/gu,'정도의 차이').replace(/\b\d{1,3}\s*[%％]/gu,'강한 정도').replace(/\b\d{1,2}\s*번\s*(?:문항|질문|답변)?/gu,'해당 상황').replace(/\s+/g,' ').trim();
  const question=answer.question.replace(/^\s*\d+\s*[.)]\s*/u,'').replace(/\b\d{1,2}\s*번\s*(?:문항|질문|답변)?/gu,'해당 상황').trim();
...
function ownerReviewReference(value:unknown){
  ...
  return value.map(item=>{
    const r=asRecord(item), verdict=asInlineText(r.ownerVerdict), text=asInlineText(r.text), feedback=asInlineText(r.ownerFeedback);
...
function confirmedFactsReference(value:unknown[]){
  return value.map(item=>{const r=asRecord(item);return Object.keys(r).length?{key:r.key,value:r.value}:item});
}
...
function rawSourceStrings(publicProfileText:string,source:PrivateDetailSource){
  return [publicProfileText,source.secretProfileText,JSON.stringify(source.ownerReview),...source.answers.flatMap(a=>[a.question,a.answer,a.reason||''])].filter(Boolean);
}
function buildSourcePacket(seed:DetailSeed,publicProfileText:string,privateSourceInput?:unknown):{packet:SourcePacket|UnknownRecord;sources:string[]}{
  if(seed.version==='detail-seed/2.0'){
    const source=privateDetailSourceSchema.parse(privateSourceInput);
    return {packet:{publicProfile:publicProfileText,secretProfile:source.secretProfileText,ownerReview:ownerReviewReference(source.ownerReview),confirmedFacts:confirmedFactsReference(source.confirmedFacts),interview:source.answers.map(semanticAnswer),coverageIndex:evidencePackReference(seed.evidencePack)},sources:rawSourceStrings(publicProfileText,source)};
  }
  return {packet:{summary:seed.summary,analysisSeeds:seed.analysisSeeds,note:'구버전 캐릭터라 저장된 해석 씨앗만 사용할 수 있음'},sources:[...seed.analysisSeeds,seed.oneLineSummary,...Object.values(seed.summary)]};
}
function allBundleText(bundle:InterpretationBundle){
  return [bundle.centralThesis,...bundle.cards.flatMap(card=>[card.thesis,...card.evidence,...card.exceptions]),...bundle.openQuestions].join(' ');
}
function allReportText(detail:DetailAnalysisGeneration){
  return [detail.outerSelf,detail.innerSelf,detail.conflictStyle,detail.affectionStyle,detail.detailedReport,...detail.coreValues,...detail.desires,...detail.fears,...detail.misunderstoodPoints,...detail.contradictions,...detail.interestingPoints].join(' ');
}
function uiArtifactReason(text:string){
  const checks:Array<[RegExp,string]>=[[/\b\d{1,2}\s*번\s*(?:문항|질문|답변)?/u,'문항 번호 노출'],[/\b\d{1,3}\s*\/\s*(?:5|100)\b/u,'점수 노출'],[/\b\d{1,3}\s*[%％]/u,'백분율 노출'],[/(?:슬라이더|백분율|선택지\s*\d+|점수\s*\d+)/u,'인터뷰 UI 표현 노출']];
  for(const [pattern,label] of checks)if(pattern.test(text))return label;return'';
}
function reportMetaReason(text:string){
  const pattern=/(?:프로필에서|공개\s*프로필|비밀\s*프로필|질문에서|답변에서|오너\s*(?:검수|답변|피드백)|AI\s*추론|원자료상|근거상|Evidence\s*Pack)/iu;
  return pattern.test(text)?'분석 과정 또는 입력 출처 노출':'';
}
function normalizeOverlap(text:string){return text.toLowerCase().replace(/[\s\p{P}\p{S}]+/gu,'')}
function hasLongVerbatimOverlap(output:string,sources:string[]){
  const target=normalizeOverlap(output);if(!target)return false;
  for(const source of sources){
    const fragments=source.replace(/\r\n?/g,'\n').split(/\n+|(?<=[.!?。！？])\s+/u);
    for(const fragment of fragments){
      const normalized=normalizeOverlap(fragment);if(normalized.length<80)continue;
      const windowLength=Math.min(90,normalized.length);
      for(let start=0;start+windowLength<=normalized.length;start+=24)if(target.includes(normalized.slice(start,start+windowLength)))return true;
    }
  }
  return false;
}
...
async function buildInterpretationBundle(seed:DetailSeed,packet:SourcePacket|UnknownRecord):Promise<InterpretationBundle>{
  ...
  for(let attempt=0;attempt<2;attempt+=1){
    const retry=attempt===0?'':`\n\n이전 결과는 너무 추상적이거나 인터뷰 UI 흔적이 남았습니다. 각 카드의 evidence에 실제 행동·상황·관계 조건을 더 보존하고 thesis는 그 근거들을 연결한 해석으로 다시 작성하세요. 실패 이유: ${lastReason}`;
    const bundle=await askClaudeJson({
      system:SYNTHESIS_SYSTEM,
      schema:interpretationBundleSchema,
      maxTokens:5200,
      input:`캐릭터 이름: ${seed.name}\n\n[분석 자료]\n${JSON.stringify(packet)}\n\n작성 규칙:\n- centralThesis는 여러 영역을 관통하는 중심 원리를 한 문단으로 적으세요.\n- cards는 6~10개를 만드세요. 각 카드에는 thesis, evidence 2~5개, exceptions 0~3개, domains, confidence가 있어야 합니다.\n- domains는 분류용 짧은 라벨이며 한국어/영어 어느 쪽도 괜찮습니다.\n- confidence는 확신 수준을 나타내는 짧은 라벨이며 한국어/영어 어느 쪽도 괜찮습니다.\n- evidence는 추상적인 형용사가 아니라 실제 행동·관계·상황 조건을 짧게 바꿔 적으세요.\n- evidence에 질문 번호나 점수는 남기지 마세요.\n- 한 카드에 설정의 구체적 행동과 인터뷰에서 드러난 판단이 함께 연결될 수 있으면 적극적으로 연결하세요.\n- 오너가 직접 정정한 내용은 가장 높은 우선순위로 반영하세요.\n- openQuestions에는 자료만으로 확정할 수 없는 해석만 남기세요.${retry}`,
      allowFallback:false,
      model:'anthropic/claude-sonnet-5',
    });
    const artifact=uiArtifactReason(allBundleText(bundle));if(artifact){lastReason=artifact;continue}return bundle;
  }
...
async function generateDetailFields(seed:DetailSeed,publicProfileText:string,privateSourceInput?:unknown):Promise<DetailAnalysisGeneration>{
  ...
  const baseInput=`캐릭터 이름: ${seed.name}\n\n[해석 지도]\n${JSON.stringify(bundle)}\n\n[구체성 확인용 참고 자료]\n${JSON.stringify(packet)}\n\n사용 순서:\n1. 해석 지도로 중심 원리와 연결 구조를 잡으세요.\n2. 참고 자료로 각 해석이 실제 캐릭터의 행동·관계·상황과 맞는지 다시 확인하세요.\n3. 해석 지도가 지나치게 일반화했거나 참고 자료와 충돌하면 참고 자료와 오너 정정 내용을 우선하세요.\n4. 최종 글에는 출처 이름이나 질문 번호를 쓰지 말고 필요한 구체적 행동과 상황만 자연스럽게 녹이세요.\n\n출력 규칙:\n- outerSelf: 타인이 반복적으로 체감하는 행동 방식과 인상을 설명하되 이 캐릭터만의 구체적 행동 패턴을 최소 하나 포함하세요. 180~320자.\n- innerSelf: 실제 선택을 정하는 우선순위와 자기인식, 긴장과 조절 방식을 설명하고 그 판단이 드러나는 구체적 상황을 포함하세요. 180~320자.\n- conflictStyle: 갈등 감지 기준 → 초기 대응 → 개입 임계점 → 고집이 생기는 조건 → 물러나는 예외를 하나의 메커니즘으로 설명하세요. 180~320자.\n- affectionStyle: 친밀감이 높아질수록 관찰·개입·보호·경계가 어떻게 달라지는지 구체적 행동과 함께 설명하세요. 180~320자.\n- coreValues / desires / fears: 각각 2~5개. 누구에게나 적용되는 단어 하나가 아니라 이 캐릭터의 반복 선택을 설명하는 구체적인 원리로 쓰세요.\n- misunderstoodPoints / contradictions / interestingPoints: 각각 2~5개. 왜 그렇게 보이는지, 어떤 두 성향이 어떤 조건에서 동시에 성립하는지까지 포함하세요.\n- detailedReport: 850~1400자. 중심 원리 하나를 잡고 관계·갈등·감정·자기인식·고유 습관이 그 원리에서 어떻게 갈라져 나오는지 연결하세요. 각 주요 문단에는 가능하면 구체적 행동·상황·관계 조건을 한두 개씩 녹이세요.\n- 자료에 없는 심리 원인이나 과거를 창작하지 마세요.\n- 문단은 논점이 실제로 바뀔 때만 \\n\\n으로 나누세요. 소제목, 번호, 불릿은 넣지 마세요.\n- 질문 번호, 점수, 퍼센트, 슬라이더 값, 선택지 번호, 분석 출처 표현은 절대 사용하지 마세요.\n\nJSON 키는 outerSelf, innerSelf, coreValues, desires, fears, conflictStyle, affectionStyle, misunderstoodPoints, contradictions, interestingPoints, detailedReport만 사용하세요.`;
  let lastReason='';
  for(let attempt=0;attempt<3;attempt+=1){
    const retry=attempt===0?'':`\n\n이전 결과는 캐릭터 고유의 구체성이 부족했거나 인터뷰 흔적이 남았습니다. 참고 자료의 구체적 행동·관계·상황을 다시 확인해 해석에 녹이되 질문 번호·점수·출처 설명·긴 직접 인용은 제거하세요. 실패 이유: ${lastReason}`;
    const raw=await askClaudeJson({system:REPORT_SYSTEM,schema:detailAnalysisRawSchema,maxTokens:7600,input:`${baseInput}${retry}`,allowFallback:false});
    const parsed=detailAnalysisGenerationSchema.safeParse(normalizeDetail(raw));if(!parsed.success){lastReason=validationReason(parsed.error);continue}
    const reportText=allReportText(parsed.data), artifact=uiArtifactReason(reportText);if(artifact){lastReason=artifact;continue}
    const meta=reportMetaReason(reportText);if(meta){lastReason=meta;continue}
