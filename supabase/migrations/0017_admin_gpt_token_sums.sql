-- Admin console: also expose the OpenAI (gpt-5.6-luna) token sums so the client
-- can estimate the gpt spend the gateway does not price (cost_usd is null for it).
-- Attribution: parse/image/questions/summary gpt -> summary path; detail gpt -> detail.
-- The client applies the gpt-5.6-luna rate ($0.20/Mtok in, $1.20/Mtok out).
